sap.ui.define([
  ...
  function getAllAliveControls() {
    return Object.assign({}, MemoryLeakCheck.oCore.mElements);
  }


  // tries to fill all control properties with string values (hoping this might trigger more leaks)
  const fillControlProperties = function(oControl) {
  ...
  const _checkControl = function(sControlName, fnControlFactory, fnSomeAdditionalFunction, bControlCannotRender) {
    QUnit.test('Control ' + sControlName + ' should not have any memory leaks', function(assert) {
      const oControl1 = fnControlFactory();
      ...
      const mPreElements = getAllAliveControls(); const oControl2 = fnControlFactory();
      ...
      oControl2.destroy();
      sap.ui.getCore().applyChanges();
      const mPostElements = getAllAliveControls();
  ...
  var detectEqualElementsInControlList = function(assert, mActual, mExpected, sMessage) {
    const aUnexpectedElements = [];

    for (const sId in mActual) {
      if (!mExpected[sId]) {
    ...
    for (let i = 0; i < aUnexpectedElements.length; i++) {
  ...
  MemoryLeakCheck.checkControl = function(sControlName, fnControlFactory, fnSomeAdditionalFunction, bControlCannotRender) {
    ...
    QUnit.module('MemoryLeakCheck.checkControl: ' + sControlName, {
      ...
      afterEach: function(assert) {
        for (const sId in MemoryLeakCheck.oCore.mElements) {
